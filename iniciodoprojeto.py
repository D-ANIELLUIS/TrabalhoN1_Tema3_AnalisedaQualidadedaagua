...
for variavel, pergunta in perguntas.items():
    ...
    # Corrigindo diferentes formas de entrada para "não"
    if resposta in ["nada", "negativo", "de jeito nenhum", "non", "nop", "no", "NÃO", "NAO" "Nao", "Não", "não", "nao", "n", "ñ"]:
        resposta = "não"
    respostas[variavel] = resposta

# Verificando se todas as respostas são "não"
todas_nao = all(resposta == "não" for resposta in respostas.values())
